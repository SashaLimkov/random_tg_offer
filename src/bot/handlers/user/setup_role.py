...
async def check_key(message: types.Message, state: FSMContext):
    ...
    if secret_key == KURATOR_SECRET_KEY:
        user: models.TelegramUser = await user_db.add_user_with_role(user_id=user_id, name=message.from_user.first_name,
                                                           role="куратор", chat_id = message.chat.id)
        await state.finish()
        await bot.send_message(
            chat_id=message.chat.id,
        ...
        user: models.TelegramUser = await user_db.add_user_with_role(user_id=user_id, name=message.from_user.first_name,
                                                                     role="наставник", chat_id=message.chat.id)
        await state.finish()
        await bot.send_message(
            chat_id=message.chat.id,
