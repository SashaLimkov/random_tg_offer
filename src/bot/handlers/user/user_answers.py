...
async def get_answer(message: types.Message):
    ...
    user: TelegramUser = await user_db.select_user(user_id=user_id)  # юзер задавший вопрос
    helper_id = message.from_user.id
    helper: TelegramUser = await user_db.select_user(user_id=helper_id)  # куратор ответивший
    ...
    if helper.state == 1 and helper.user_role == "куратор":
        await user_db.update_user_state(
            user_id=helper_id,
            state=0)  # поставили состояние у куратора "отвечает на вопрос"

        await question_db.add_helper(user=user, helper_id=helper_id)  # Добавили вопросу id отвечающего на него
    ...
    await question_db.add_history(user=user,pk=question.pk, history=history)  # записали историю в бд
    ...
    try:
        await bot.edit_message_reply_markup(
            chat_id=user.user_id,
            message_id=user_data[user.user_id],
            reply_markup=None
        )
...
async def answer_done(call: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(chat_id=user_id, text="Поставьте оценку", reply_markup=await ik.set_kurators_rate())
