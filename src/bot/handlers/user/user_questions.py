...
async def send_user_questions(call: types.CallbackQuery, state: FSMContext):
    ...
    if user.user_role == "ученик":
        ...
        for m in m_list:
            ...
            sent_q_id_dict[m_list[m]] = m.message_id + 1
            await bot.delete_message(chat_id=m_list[m], message_id=m.message_id)
...
async def continue_question(message: types.Message):
    ...
    if user.user_role == "ученик":
        await bot.send_message(
            chat_id=user_id,
            text=f"История ваших вопросов на данный момент выглядит так:\n{history.replace('Q: ', '- ')}",
        )
        for kur in k_list:
            if kur in sent_q_id_dict:
                await bot.send_message(
                    chat_id=kur,
                    text=text.format(clear_q),
