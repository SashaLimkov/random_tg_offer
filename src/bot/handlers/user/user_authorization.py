...
async def get_profile_panel(message: types.Message):
    ...
    try:
        number = cur.execute(
            "SELECT number FROM data WHERE id == ?", (message.from_user.id,)
        ).fetchone()[0]
        if number:
...
async def check_phone(message: types.Message, state: FSMContext):
    ...
    if await is_phone_number_valid(phone_number):
        ...
        try:
            authorizted = cur.execute(
                "SELECT state FROM data WHERE id = ?", (message.from_user.id,)
            ).fetchone()[0]
            if authorizted == 1:
        ...
        base.commit()
        user_type = cur.execute(
            "SELECT role FROM data WHERE id = ?", (message.from_user.id,)
        ).fetchone()[0]
        result = dict(r.json())
        await _role_segregated_menu(message, result, user_type)
    else:
        await bot.send_message(chat_id=message.chat.id, text=td.INVALID_PHONE)
...
async def _role_segregated_menu(message: types.Message, result, user_type):
    ...
    if result["user"] and result["is_active"]:
        if user_type == "ученик":
            cur.execute(
                "UPDATE data SET state == ? WHERE id == ?", (1, message.from_user.id)
            )
            base.commit()
            await bot.send_message(
                chat_id=message.chat.id,
